...
function updateFaucetInfo(data) {
  ...
  document.getElementById("faucetAmount").textContent = formatAmount(
    data.amount_per_request,
  );

  // Update statistics
  document.getElementById("faucetBalance").textContent = formatAmount(
    data.balance,
  );
  document.getElementById("totalDistributed").textContent = formatAmount(
    data.total_distributed,
  );
  document.getElementById("uniqueRecipients").textContent =
    data.unique_recipients.toLocaleString();
  document.getElementById("last24h").textContent =
    data.requests_last_24h.toLocaleString();
}
...
async function loadRecentTransactions() {
  ...
  try {
    ...
    container.innerHTML = state.recentTransactions
      .map(
        (tx) => `
...
function validateAddress() {
  ...
  // Check if address matches pattern (aura1 prefix for mainnet/testnet)
  const addressPattern = /^aura1[a-z0-9]{38,58}$/;
  if (!addressPattern.test(address)) {
    errorElement.textContent =
      'Invalid AURA address format. Address must start with "aura1"';
    errorElement.classList.add("show");
    return false;
  }
